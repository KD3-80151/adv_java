...
public class UserDao extends Dao implements UserDaoInterface{
	...
	public User findByEmail(String email) throws Exception{
		...
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			...
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					User us = new User(rs.getInt("id"), rs.getString("first_name"),
	...
	public int editProfile (User u)throws Exception{
				String sql = "update users set first_name =?, last_name =?, mobile=?, birth=?, "
	...
	public List<Movies> displayNonEditedMovies (User u) throws Exception{
		...
		String sql = "select m.movie_id,title from movies m left outer join reviews r on m.movie_id"
				+ " = r.movie_id and r.user_id =? where r.review is null";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
	...
	public List<Movies> displayEditedMovies (User u) throws Exception{
		List<Movies> l = new ArrayList<>();
		String sql = "select m.movie_id,title from movies m left outer join reviews r on m.movie_id ="
				+ " r.movie_id and r.user_id =? where r.review is not null";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
	...
	public List<Reviews> AllReview (User u) throws Exception{
		...
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					Reviews rm = new Reviews(rs.getInt("id"), rs.getInt("movie_id"), 
							rs.getString("review"), rs.getInt("rating"), rs.getInt("user_id"), 
	...
	public Reviews findReview (int id) throws Exception{
		...
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			...
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					Reviews r = new Reviews(rs.getInt("id"), rs.getInt("movie_id"), 
							rs.getString("review"), rs.getInt("rating"), rs.getInt("user_id"), 
							rs.getTimestamp("modified"));
					return r;
				}
	...
	public List<Review_Movie> displayAllReviews  () throws Exception{
		...
		String sql = "select r.id review_id,m.id movie_id,title,rating,modified,review,user_id "
				+ "from movies m,reviews r where m.id = r.movie_id";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					Review_Movie rm = new Review_Movie(rs.getInt("review_id"),rs.getInt("movie_id"),
							rs.getString("review"),rs.getInt("rating"),rs.getTimestamp("modified"),
							rs.getString("title"),rs.getInt("user_id"));
					l.add(rm);
				}
			}
		}
	...
	public List<Review_Movie> displayMyReviews  (User u) throws Exception{
		List<Review_Movie> l = new ArrayList<>();
		String sql = "select r.id review_id,m.id movie_id,title,rating,modified,review,user_id "
				+ "from movies m,reviews r where m.id = r.movie_id and r.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
	...
	public List<Reviews> displayMyReviews2  (User u) throws Exception{
		List<Reviews> l = new ArrayList<>();
		String sql = "select review_id,rating,modified,review from movies m,reviews r where m.id = "
				+ "r.movie_id and r.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					Reviews rm = new Reviews(rs.getInt("review_id"), 0,rs.getString("review"), 
							rs.getInt("rating"), 0,rs.getTimestamp("modified"));
					l.add(rm);
				}
			}
		}
	...
	public List<User> displayUser (User u) throws Exception{
		...
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			...
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					User us = new User(rs.getInt("user_id"), rs.getString("fname"), rs.getString("lname"), 
	...
	public List<Review_Movie> displaySharedReviews (User u) throws Exception {
		List<Review_Movie> l = new ArrayList<>();
		String sql = "select r.id review_id,m.id movie_id,title,rating,modified,review,s.user_id "
				+ "from reviews r,shares s,movies m where r.id = s.review_id and r.movie_id = m.id "
				+ "and s.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
