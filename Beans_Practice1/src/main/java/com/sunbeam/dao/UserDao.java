...
public class UserDao extends Dao implements UserDaoInterface{
	...
	public List<Review_Movie> displayAllReviews  () throws Exception{
		...
		String sql = "select title,rating,modified,review,user_id from movies m,reviews r where m.id = r.movie_id";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					Review_Movie rm = new Review_Movie(rs.getString("review"),rs.getInt("rating"),rs.getTimestamp("modified"),rs.getString("title"),rs.getInt("user_id"));
					l.add(rm);
				}
			}
		}
	...
	public List<Review_Movie> displayMyReviews  (User u) throws Exception{
		List<Review_Movie> l = new ArrayList<>();
		String sql = "select title,rating,modified,review,user_id from movies m,reviews r where m.id = r.movie_id and r.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
	...
	public List<Review_Movie> displaySharedReviews (User u) throws Exception {
		List<Review_Movie> l = new ArrayList<>();
		String sql = "select title,rating,modified,review,s.user_id from reviews r,shares s,movies m where r.id = s.review_id and r.movie_id = m.id and s.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
