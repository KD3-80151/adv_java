...
public class UserDao extends Dao{
	...
	public List<Movies> displayEditedMovies (User u) throws Exception{
		...
		String sql = "select m.movie_id,title from movies m left outer join reviews r "
				+ "on m.movie_id = r.movie_id and r.user_id =? where r.review is not null";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
	...
	public List<Review_Movie> displayAllReviews  () throws Exception{
		...
		String sql = "select title,rating,modified,review from movies m,reviews r where "
				+ "m.movie_id = r.movie_id";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					Review_Movie rm = new Review_Movie(rs.getString("review"),
							rs.getInt("rating"),rs.getTimestamp("modified"),rs.getString("title"));
					l.add(rm);
				}
			}
		}
	...
	public List<Review_Movie> displayMyReviews  (User u) throws Exception{
		List<Review_Movie> l = new ArrayList<>();
		String sql = "select title,rating,modified,review from movies m,reviews r where"
				+ " m.movie_id = r.movie_id and r.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
	...
	public List<Reviews> displayMyReviews2  (User u) throws Exception{
		...
		String sql = "select review_id,rating,modified,review from movies m,"
				+ "reviews r where m.movie_id = r.movie_id and r.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					Reviews rm = new Reviews(rs.getInt("review_id"), 0,
							rs.getString("review"), rs.getInt("rating"), 0,
	...
	public List<Review_Movie> displaySharedReviews (User u) throws Exception {
		List<Review_Movie> l = new ArrayList<>();
		String sql = "select title,rating,modified,review from reviews r,shares s,"
				+ "movies m where r.review_id = s.review_id and r.movie_id = m.movie_id "
				+ "and s.user_id =?";
		try (PreparedStatement stmt = con.prepareStatement(sql)){
			stmt.setInt(1, u.getId());
			try (ResultSet rs = stmt.executeQuery()){
